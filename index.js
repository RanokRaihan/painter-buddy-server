...
client.connect((err) => {
  ...
  app.get("/singleService/:id", (req, res) => {
    ...
    serviceCollection.find({ _id: ObjectId(req.params.id) }).toArray((err, document) => {
      res.send(document);
    });
  });
  //varify admin
  app.get("/varifyAdmin/:email", (req, res) => {
    ...
    adminCollection.find({ admin: adminEmail }).toArray((err, document) => {
  ...
  app.get("/userOrder/:email", (req, res) => {
    ...
    allBookings.find({ email: email }).toArray((err, document) => {
      res.send(document);
    });
  });
  ...
  app.get("/singleBooking/:id", (req, res) => {
    allBookings.find({ _id: ObjectId(req.params.id) }).toArray((err, document) => {
      res.send(document);
    });
  });
  ...
  app.post("/placeOrder", (req, res) => {
    const service = req.body;
    allBookings.insertOne(service).then((result) => {
      console.log(result.insertedCount);
      res.send(result.insertedCount > 0);
    });
  });
  ...
  app.delete("/deleteService/:id", (req, res) => {
    ...
    console.log(id);
    serviceCollection.deleteOne({ _id: id }).then((document) => res.send("One Service Deleted"));
  });
});
